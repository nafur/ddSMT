...
def matches_golden(golden, run, ignore_out, match_out, match_err):
    ...
    Otherwise, if either :code:`match_out` or :code:`match_err` are given, they need to match.
    Otherwise, both stdout and stderr must be the same.
    """
...
def check(filename):
    ...
    First execute the command and then call :code:`matches_golden`.
    If a cross-check command is specified, do the same for that one as well."""
...
def do_golden_runs():
    ...
    logging.info("")
    logging.info(f'starting initial run{", cross checking" if options.args().cmd_cc else ""}...')
    logging.info("")
    ...
    if options.args().match_out:
        if options.args().match_out not in __GOLDEN.out:
            logging.error(f'Expected stdout to match "{options.args().match_out}"')
            sys.exit(1)

    if options.args().match_err:
        if options.args().match_err not in __GOLDEN.err:
            logging.error(f'Expected stderr to match "{options.args().match_err}"')
            sys.exit(1)

    if options.args().timeout is None:
        options.args().timeout = round((__GOLDEN.runtime + 1) * 1.5, 2)
        logging.info(f'automatic timeout: {options.args().timeout:.2f} seconds')
    ...
    if options.args().cmd_cc:
        ...
        if options.args().match_out_cc:
            logging.info(f'match (cc) (stdout): "{options.args().match_out_cc}"')
        if options.args().match_err_cc:
            logging.info(f'match (cc) (stderr): "{options.args().match_err_cc}"')

        if options.args().timeout_cc is None:
            options.args().timeout_cc = round((__GOLDEN_CC.runtime + 1) * 1.5, 2)
            logging.info(f'automatic timeout (cc): {options.args().timeout_cc:.2f} seconds')
