...
def test_bool_de_morgan():
    ...
    exprs = {
        Node('not', ('and', 'x', 'y')): [Node('or', ('not', 'x'), ('not', 'y'))],
        Node('not', ('or', 'x', 'y')): [Node('and', ('not', 'x'), ('not', 'y'))],
    }
    for e in notfilter:
...
def test_bool_eliminate_false_equality():
    ...
    exprs = {
        ...
        Node('=', 'x', 'false', 'y'): [Node('and', ('not', 'x'), ('not', 'y'))],
    }
    for e in notfilter:
        assert not m.filter(e)
    for k, v in exprs.items():
...
def test_bool_eliminate_implication():
    ...
    exprs = {
        ...
        Node('=>', 'x', 'y', 'z'): [Node('and', ('or', ('not', 'x'), 'y'), ('or', ('not', 'y'), 'z'))],
    }
    for e in notfilter:
        assert not m.filter(e)
    for k, v in exprs.items():
...
def test_bool_negate_quantifier():
    ...
    exprs = {
        Node('not', ('forall', ('Real', 'x'), 'y')): [Node('exists', ('Real', 'x'), ('not', 'y'))],
        Node('not', ('exists', ('Real', 'x'), 'y')): [Node('forall', ('Real', 'x'), ('not', 'y'))],
    }
    for e in notfilter:
        assert not m.filter(e)
    for k, v in exprs.items():
...
def test_bool_xor_remove_constant():
    ...
    exprs = {
        ...
        Node('xor', 'true', 'y'): [Node('xor', 'y'), Node('not', ('xor', 'y'))],
        Node('xor', 'true', 'y', 'z', 'true'): [Node('xor', 'y', 'z'), Node('not', ('xor', 'y', 'z'))],
    }
    for e in notfilter:
        assert not m.filter(e)
    for k, v in exprs.items():
