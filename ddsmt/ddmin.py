...
def _subst(exprs, subset, mutator):
    """Return list of mutated formulas.
       ...
       nodes in :code:`exprs`.
       At granularity 1 try all mutations returned by :code:`mutator`, for all
       other granularities pick the first mutation.
    """
...
def _worker(task):
    """Perform mutations on given :code:`subset` and check mutated input.
       ...
       A worker process takes a task and performs mutations on the input and
       checks whether the input fails.
    """
