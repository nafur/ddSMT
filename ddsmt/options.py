...
class CustomFormatter(argparse.ArgumentDefaultsHelpFormatter, argparse.HelpFormatter):
    ...
    Uses :code:`argparse.ArgumentDefaultsHelpFormatter` to print default values, but avoids printing for :code:`ToggleAction` options and :code:`None` defaults.
    Furthermore uses the :code:`argparse.HelpFormatter`,
    ...
    def _get_help_string(self, action):
        """Render the help string. Appends the default, if needed."""
...
def parse_options(mutators):
    ...
    ap.add_argument('--pretty-print', action='store_true', default=False, help='pretty-print to output file')
    ap.add_argument('--wrap-lines', action='store_true', default=False, help='wrap lines in output file')
    ...
    apcheck.add_argument(
        ...
        "(default: 1.5 * golden runtime)")
    apcheck.add_argument("--ignore-output-cc",
                         action="store_true",
                         help="ignore stdout and stderr, only consider exit code for cross check command")
    apcheck.add_argument("--match-err-cc",
                         metavar='str',
                         help="match string in stderr to identify failing input for cross check command")
    apcheck.add_argument("--match-out-cc",
                         metavar='str',
                         help="match string in stdout to identify failing input for cross check command")
