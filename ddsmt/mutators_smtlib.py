...
class EliminateVariable:
    ...
    Tries to globaly replace every leaf node from an equality by every other
    node from the same equality."""
    def filter(self, node):
...
class SimplifyQuotedSymbols:
    ...
    def filter(self, node):
        return is_quoted_symbol(node) and re.match('\\|[a-zA-Z0-9~!@$%^&*_+=<>.?/-]+\\|', node.data) is not None
