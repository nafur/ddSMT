...
class BVExtractZeroExtend:
    """Simplifies an :code:`extract` of a :code:`zero_extend` by pushing the :code:`zero_extend` to the outside and
    reducing the bit-widths, if possible."""
    def filter(self, node):
        return is_indexed_operator(node, 'extract', 2) and is_indexed_operator(node[1], 'zero_extend')
    ...
    def mutations(self, node):
        ...
        return [Node(
            ('_', 'zero_extend', str(upper - varwidth + 1)),
...
def get_mutators():
    """
    Return mapping from mutator class names to the name of their config options.
    """
    return {
