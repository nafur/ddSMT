...
def get_bv_constant_value(node):
    """Assume that node is a bit-vector constant and return :code:`(value, bit-width)`."""
...
def possible_bv_widths_imp(definition):
    ...
    if not is_leaf(definition):
        return [
            w for arg in definition for w in possible_bv_widths_imp(arg)
        ]
    return []


def possible_bv_widths(node):
    """Return the bit-widths this node may have. First try to get the type of the given node, then just collect bit-width that show up elsewhere in the input."""
...
class BVConcatToZeroExtend:
    ...
    def mutations(self, node):
        return [Node((
            '_', 'zero_extend',
            get_bv_constant_value(node[1])[1]
        ), node[2])]
...
class BVEvalExtend:
    """Evaluates a bit-vector :code:`(sign|zero)_extend` if it is applied to a constant or another :code:`(sign|zero)_extend`."""
    def filter(self, node):
...
class BVOneZeroITE:
    ...
    def filter(self, node):
        ...
        if not has_name(node[1]) or get_name(node[1]) != '=' or len(node[1]) != 3:
            return False
        if not is_bv_constant(node[2]) or get_bv_constant_value(node[2]) != (1, '1'):
            return False
        if not is_bv_constant(node[3]) or get_bv_constant_value(node[3]) != (0, '1'):
            return False
        return True
