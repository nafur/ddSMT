...
class Node:
    ...
    The :code:`id` is automatically set to a unique integer that can be used for local substitutions.
    """
    __slots__ = 'id', 'data'
    __ID_COUNTER = 0
...
def render_smtlib(exprs):
    ...
    if options.args().pretty_print:
        # pretty print
        return '\n'.join(map(lambda expr: __render_smtlib_expression(expr, True), exprs))
