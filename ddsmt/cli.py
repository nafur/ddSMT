...
def check_options():
    ...
    if not os.path.isfile(options.args().cmd[0]):
        raise DDSMTException('Command "{}" is not a regular file'.format(options.args().cmd[0]))
...
def ddsmt_main():
    ...
    if reduced_exprs != exprs:
        ...
        logging.debug("main process:   {:.2f} s".format(
            time.process_time() - start_time_process))
